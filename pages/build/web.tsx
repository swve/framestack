...
const Web = props => {
  return (
    <Layout
      title="Build web apps and websites"
      desc="Frameworks, libraries and tools to help you make website and web apps"
    >
      <div className="hero-build">
        <span className="badge">Build</span>
        <HeroLinks title="Web Apps" ></HeroLinks>
        <div className="types">
          <Link href="?">
            <a>
...
Web.getInitialProps = async function(context) {
  ...
  if (!type) {
    var result = provider.getByTags("web", p ? p : 0);
  } else {
    var result = provider.getBy("web", p ? p : 0, type);
  }
