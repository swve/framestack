...
const Web = props => {
  return (
    <Layout
      title="Build Desktop apps and software"
      desc="Frameworks, libraries and tools to help you make desktop software"
    >
      <div className="hero-build">
        <span className="badge">Build</span>
        <HeroLinks title="Software" ></HeroLinks>
        <div className="types">
          <Link href="?">
            <a>
...
Web.getInitialProps = async function(context) {
  ...
  if (!type) {
    var result = provider.getByTags("software", p ? p : 0);
  } else {
    var result = provider.getBy("software", p ? p : 0, type);
  }
